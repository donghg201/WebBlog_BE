...
public interface DocumentRepository extends JpaRepository<Document, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "insert into `my_project`.`document`(`document`.`document_name`,`document`.`document_writer`,`document`.`document_poster`,`document`.`document_date`,`document`.`description`,`document`.`delete_flag`) values (?1, ?2, ?3, ?4, ?5, ?6);", nativeQuery = true)
    void createDocument(String documentName, String documentWriter, String documentPoster, String documentDate, String description, Integer deleteFlag);
